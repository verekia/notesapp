...
Migrations can be generated by Hasura's UI console as you click around to create tables and columns, but I prefer a more robust and cleaner way to decribe my database schema directly with code. That's why I use Prisma and its declarative schema, only to take care of migrations. Note that in this architecture, I am not using any [ORM](#data-access-orms), only Hasura's API.
